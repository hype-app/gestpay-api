...
| **`acquirerBIN`** | 11 |This field are required only if `transTypeReq` value is `Y`. Acquiring institution identification code as assigned by the DS receiving the AReq  message |
| **`acquirerMerchantID `** | 35 | This field are required only if `transTypeReq` value is `Y`. Acquirer-assigned Merchant identifier. This may be the same value that is used in authorisation requests sent on behalf of the 3DS Requestor and is represented in ISO 8583 formatting requirements. | 
